...
Write access denied with storage.read:/
    ${endpointpath}   GET SE endpoint path   ${se_alias}
    ${token}   Get token   scope=-s storage.read:${endpointpath}
    ${uuid}   Generate UUID
    ${url}   SE URL   robot-test-${uuid}
    ${rc}   ${out}   Curl Put Error   /etc/services  ${url}
    Should Contain   ${out}   403
...
Path authorization enforced on storage.read
    ${endpointpath}   GET SE endpoint path   ${se_alias}
    ${token}   Get token   scope=-s storage.read:${endpointpath}/wlcg-jwt-compliance
...
Path authorization enforced on storage.write
    ${endpointpath}   GET SE endpoint path   ${se_alias}
    ${token}   Get token   scope=-s storage.modify:${endpointpath}/wlcg-jwt-compliance
    ${endpoint}   GET SE endpoint   ${se_alias}
    ${uuid}   Generate UUID
    ${url}   Set Variable   ${endpoint}/not-found-${uuid}
    ${rc}   ${out}   Curl Put Error   /etc/services  ${url}
...
storage.modify does not imply storage.read
    ...
    Should Match Regexp   ${out}   20[01]
    ${endpointpath}   GET SE endpoint path   ${se_alias}
    ${token}   Get token   scope=-s storage.modify:${endpointpath}
    ${rc}   ${out}   Curl Error   ${url}
    Should Contain   ${out}   403
...
storage.create does not imply storage.read
    ...
    Should Match Regexp   ${out}   20[01]
    ${endpointpath}   GET SE endpoint path   ${se_alias}
    ${token}   Get token   scope=-s storage.create:${endpointpath}
    ${rc}   ${out}   Curl Error   ${url}
    Should Contain   ${out}   403
